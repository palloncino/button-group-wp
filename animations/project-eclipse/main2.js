document.addEventListener("DOMContentLoaded", () => {
  ...
  function phase2Animation(scrollPos) {
    ...
    Object.keys(atoms).forEach((key) => {
      let atom = atoms[key];
      let movement = atomMovements[key]?.phase2;
      if (movement) {
        atom.x = interpolate(movement.startX, movement.endX, progress);
        atom.y = interpolate(movement.startY, movement.endY, progress);
      }
    });
